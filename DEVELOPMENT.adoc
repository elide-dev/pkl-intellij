...
=== Publishing to our custom plugin repository

`./gradlew buildPlugin` builds the distribution into `build/distributions`, then `tools/deploy.sh` uploads it to our
custom plugin repository. All metadata (plugin id, name, description, vendor, version, since/until build) is read from
the patched `plugin.xml` inside the built archive, so the upload always matches what the IDE will see.

The script requires `xmllint`, `jq`, `curl` and `unzip`, plus these environment variables:
